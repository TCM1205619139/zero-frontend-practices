...
- 新增 Vue SFC 不使用 `<script setup lang="ts">`；本项目不新增 TypeScript。
...
- 页面私有组件默认只服务当前 feature；跨 feature 复用前先抽到公共组件或明确依赖边界。
- 不为了统一风格改动历史 auto-import；新增复杂业务模块优先保持可读性，重要业务依赖显式 import。
...
- 复杂业务状态优先抽成语义化 getter、computed 或 helper，不在模板中散落原始状态值、planCode 或 magic string。
- 动态组件可以返回稳定空对象，避免为了模板安全写大量 `v-if`；但不要让空对象掩盖真实异常。
- 组件事件命名表达业务意图，例如 `submit`、`close`、`renew`、`complete`。
- 组件 props 命名表达业务含义，避免在复杂业务组件中使用过泛的 `data`、`info`、`item`。
- 子组件通过 emit 表达业务动作或结果，不直接修改父级状态。
...
- 用户主动触发的异步动作需要有 loading / disabled 边界，并确保结束时复位。
- 轮询、定时器、SSE、websocket listener、DOM/window 事件监听等副作用需要在卸载或作用域结束时清理。
...
- 不要为了润色随意修改已有 i18n key；改文案前确认翻译资源、业务语义和影响页面。
